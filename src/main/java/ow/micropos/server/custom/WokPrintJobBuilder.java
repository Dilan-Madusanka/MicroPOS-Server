...
public class WokPrintJobBuilder {
    ...
    public PrintJob order(SalesOrder curr, String notification, boolean suppressAddPrefix) {
        byte[] data = reset()
                .initialize()
                ...
                .text("---------------------")
                .feed(1)
                .optional(curr.getCookTime() != null,
                        () -> font(EscPosBuilder.Font.DWDH_EMPHASIZED)
                                .text("Cook Time : " + cookFormat.format(curr.getCookTime()))
                                .feed(1)
                                .font(EscPosBuilder.Font.DWDH)
                                .text("---------------------")
                                .feed(1)
                ...
                .feed(1)
                .align(EscPosBuilder.Align.LEFT)
                .forEach(curr.getProductEntries(), pe -> productEntry(pe, suppressAddPrefix))
                .feed(5)
    ...
    private void productEntry(ProductEntry pe, boolean suppressAddPrefix) {

        ProductEntryStatus status = pe.getStatus();
        String mainLine = "";

        if (status == ProductEntryStatus.SENT) {
            font(EscPosBuilder.Font.DWDH);
            mainLine += "   ";
        ...
        } else if (status == ProductEntryStatus.REQUEST_SENT) {
            font(EscPosBuilder.Font.DWDH_EMPHASIZED);
            if (suppressAddPrefix)
                mainLine += "   ";
            else
                mainLine += "*A ";
    ...
    @Deprecated
    // All the logic should be performed prior to building request.
